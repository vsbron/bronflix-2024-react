...
import { GenreMediaPreviewProps, Media } from "@/lib/types";
...
function GenreMediaPreview({ media, type }: GenreMediaPreviewProps) {
  // Getting all the necessary data for the preview
  const { mediaTitle, mediaImage } = getSearchMediaData(media);
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className={`flex ${BASE_GAP_CLASS} text-[1.5rem]`}>
          <IconWrapper icon={<CalendarIcon />}>{formatDate(date)}</IconWrapper>
...
export default GenreMediaPreview;
